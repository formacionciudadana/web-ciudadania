...
my_bibfile <- "content/tesistas/tesis-ciudadania.bib"
out_fold   <- "content/tesistas"
...
md_files<- list.files(path = "content/tesistas",pattern = ".md")[-1] %>%
  str_replace_all(pattern = ".md",replacement = "") %>%
  as.list()

for (j in 1:length(md_files)){
  folder <- dir.create(paste0("content/tesistas/",md_files[j]))
}

# Move created .md files to new directories -----------------------------------

#list of md files
files<- list.files(path = "content/tesistas",pattern = ".md",full.names = T)[-1]

#list of cite keys/names of the md files for merge
citekey <- list.files(path = "content/tesistas",pattern = ".md",full.names = F)[-1] %>%
  str_replace_all(pattern = ".md",replacement = "")
#list of folder directory for each md file
dir_folder <-
  list.dirs(path = "content/tesistas",recursive = F)
dir_folder<- dir_folder[!(dir_folder=="content/tesistas/example")] #clean example file

# check for url of the featured image
featured <-
  RefManageR::ReadBib(file = "content/tesistas/tesis-ciudadania.bib",
                      check = "warn",
                      .Encoding = "UTF-8") %>%
  data.frame() %>%
...
df_move$orig_name <- paste0(
  df_move$dir_folder,
  str_remove_all(string = df_move$files,pattern ="content/tesistas")
  )
